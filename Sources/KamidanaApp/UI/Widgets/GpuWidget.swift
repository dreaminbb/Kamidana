...
struct GpuWidget: View {
    ...
    var body: some View {
        ...
        let usageColor =
            gpu.map { color(for: $0.activeRatio, colors: colors) }
        ...
        WidgetActionButton(action: { interaction.activate(activation) }) {
            FormattedWidgetLabel(
                ...
                values: ["usage": usage],
                iconColor: theme?.iconForeground ?? usageColor,
                textColor: theme?.foreground ?? usageColor
            )
        }
    ...
    private func popoverContent(colors: GlobalColorsConfig, gpu: GPUUsageInfo?) -> some View {
        VStack(alignment: .leading, spacing: 14) {
            ...
            VStack(alignment: .leading, spacing: 5) {
                detailRow(
                    label: "Usage",
                    value: gpu.map { String(format: "%.1f%%", $0.activeRatio) } ?? "Unavailable",
                    colors: colors)
                detailRow(
                    label: "Status", value: gpu == nil ? "Unsupported or unavailable" : "Available",
                    colors: colors)
                detailRow(
                    label: "Thermal", value: matrix.data.thermalState ?? "Unavailable",
                    colors: colors)
            }
                    ...
                    ForEach(processes.prefix(SystemMatrix.topProcessLimit)) { process in
                        HStack(spacing: 7) {
                            ...
                            Text(String(format: "%.1f%%", process.gpuUsage))
                                .foregroundColor(
                                    color(for: Float(process.gpuUsage), colors: colors)
                                )
                                .monospacedDigit()
                        }
                        .font(.system(size: 12, design: .monospaced))
