        ...
        if let battery = matrix.data.batteryUsage {
            ...
            .widgetPopup(
                ...
                VStack(alignment: .leading, spacing: 12) {
                    ...
                    Text("Battery").font(.subheadline).foregroundColor(
                    ...
                    if let watt = battery.wattInfo {
                        VStack(alignment: .leading, spacing: 8) {
                            HStack {
                                Text("Power:").foregroundColor(Color(hex: colors.textSecondary))
                                    .frame(
                            ...
                            HStack {
                                Text("Voltage:").foregroundColor(Color(hex: colors.textSecondary))
                                    .frame(
                                        width: 70, alignment: .leading)
                                Text(String(format: "%.1f V", watt.voltage / 1000)).foregroundColor(
                                    Color(hex: colors.textPrimary))
                            }
                            HStack {
                                Text("Amperage:").foregroundColor(Color(hex: colors.textSecondary))
                                    .frame(
                                        width: 70, alignment: .leading)
                    ...
                    if let thermal = matrix.data.thermalState {
                        ...
                        Text("Thermal").font(.subheadline).foregroundColor(
                            Color(hex: colors.textSecondary))
