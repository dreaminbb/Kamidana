...
final class ConfigManagerTests: XCTestCase {
    ...
    func testResolveConfigDirectory() {
        let expectedURL = defaultHomeURL
        ...
        XCTAssertEqual(actualURL.path, expectedURL.path, "Config directory must be fixed to ~/.config/kamidana")
    }
    ...
    func testResolveConfigDirectory_CustomHome() {
        ...
        XCTAssertEqual(actualURL.path, expectedURL.path, "Config directory for custom home should be /Users/testuser/.config/kamidana")
    }

    /// Tests resolving the fixed config.yaml file URL (~/.config/kamidana/config.yaml)
    func testResolveConfigFileURL() {
        let expectedFileURL = defaultHomeURL
            .appendingPathComponent(".config")
            .appendingPathComponent("kamidana")
        ...
        XCTAssertEqual(actualFileURL.path, expectedFileURL.path, "Config file URL must be fixed to ~/.config/kamidana/config.yaml")
    }
    ...
    func testFetchUserConfigPath() {
        ...
        XCTAssertTrue(pathString.hasSuffix(".config/kamidana"), "Config path string must end with .config/kamidana")
    }
}
