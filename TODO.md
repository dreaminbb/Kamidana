    ...
    - [ ] 全Widgetの通常・空・無効・読み込み・エラー・展開状態UIをアプリ側で定義し、ユーザーと実機合格するまで反復
        ...
        - [ ] 1状態ずつ実装・自動テスト・ユーザー実機確認を行う
            ...
            - [ ] Network Widgetへ接続情報と通信速度UIを統合
                ...
                - [ ] ユーザー実機確認で合格
            - [ ] CPU Widgetのコアグラフ・詳細・スクロール可能な上位プロセス一覧
                - [x] コア別使用率が利用可能な場合にグラフを表示
                - [x] コア別使用率がない場合も詳細と上位プロセスを表示
                - [x] 展開時に上位20プロセスをスクロール表示
                - [x] ユーザー実機確認で合格
            - [ ] GPU Widgetの詳細・未対応ハードウェア状態
                - [x] GPU使用率と温度状態を展開UIへ表示
                - [x] 未対応または利用不可時もコンパクトUIと詳細を表示
                - [x] IORegistryの累積GPU時間からプロセス別GPU使用率を表示
                - [ ] ユーザー実機確認で合格
            - [ ] 通常ウィジェットのパネル全体を操作領域にする
                - [x] CPU・GPU・Network・Memory・Disk・Battery・Bluetooth・Customを共通ButtonStyleへ統一
                - [ ] ユーザー実機確認で合格
            - [ ] 情報ウィジェットの表示アクションを統一する
                - [x] `activate: hover | click`を実行時へ伝播
                - [x] CPU・GPU・Memory・Diskの既定アクションをhoverに統一
                - [x] NetworkはWi-Fi操作を維持するためExampleでclickを指定
                - [ ] ユーザー実機確認で合格
                - [x] 展開パネルへのホバー中は表示を維持してプロセス一覧をスクロール可能にする
